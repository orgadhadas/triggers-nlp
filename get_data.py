...
def main():
    ...
    datasets_kind = 'dev' # should be dev or train
    if datasets_kind == 'dev':
            dataset = reader.read('https://s3-us-west-2.amazonaws.com/allennlp/datasets/snli/snli_1.0_dev.jsonl')
    else:
            dataset = reader.read('https://s3-us-west-2.amazonaws.com/allennlp/datasets/snli/snli_1.0_train.jsonl')
    model = load_archive('https://allennlp.s3-us-west-2.amazonaws.com/models/esim-glove-snli-2019.04.23.tar.gz').model
    # model.eval().cuda()
    vocab = model.vocab

    label = 'neutral'
    f_without_trigger = open(f"{datasets_kind}_data_label_{label}.txt", "w+")
